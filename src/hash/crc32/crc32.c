...
#if !defined(NP_GOODIES_CRC32_DISABLE_LOOKUP_TABLE) && !defined(NP_GOODIES_CRC32_ENABLE_BRANCHLESS)
/**
 * Pre-computed lookup tables for CRC32B hashes that helps with performances
 *  at the cost of 1 KiB of space for the table itself.
 */
const static uint32_t crc32b_table[256] = {
...
uint32_t hash_crc32b(const char *data, size_t data_size) {
	...
	for(size_t data_offset = 0; data_offset < data_size; data_offset++) {
		#if defined(NP_GOODIES_CRC32_DISABLE_LOOKUP_TABLE)
		...
		#elif defined(NP_GOODIES_CRC32_ENABLE_BRANCHLESS)
		crc32 ^= data[data_offset];
		...
		#else
		crc32 = (crc32 >> 8) ^ crc32b_table[(data[data_offset] ^ crc32) & 0xFF];
		#endif
	}
